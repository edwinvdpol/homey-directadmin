...
class DomainDevice extends Device {
    ...
    async _syncDevice () {
        try {
            ...
            var result = await this.api.getAdditionalDomains(data.id);
    ...
    async _getBandwidthSetting (domain) {
        var response = (domain.bandwidth > 0 ? pretty((domain.bandwidth * bytesInMb)) : '0');
    ...
    async _getQuotaSetting (domain) {
        var response = (domain.quota > 0 ? pretty((domain.quota * bytesInMb)) : '0');
    ...
    async _getEmailQuotaSetting (quota) {
        var response = (quota > 0 ? pretty((quota * bytesInMb)) : '0');
